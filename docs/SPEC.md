...
│   ├── react.sh             # ReAct 解析器 (单轮模式使用，多行结构化解析)
...
├── prompts/
│   ├── system.sh            # System prompt 模板 (单轮 ReAct 模式)
...
Loop Agent 采用**单对话流 + Function Calling** 架构，取代旧版 planner + react 双循环设计。规划能力通过 `plan_step` 工具实现，而非独立规划器模块。
...
v0.2 将 Loop Agent 从 planner + react 双循环架构重构为 Function Calling 单对话流架构：
...
| 9 | 移除 prompts/react_format.sh | Function Calling 不需要 ReAct 格式说明 |
